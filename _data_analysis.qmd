...
Running `SThet` takes 12-15 minutes on our institutional HPC with 2 cores for 4 genes across the 6 samples (though only 5 of the 6 run successfully. The sixth fails silently). And `get_gene_meta()` takes a couple extra minutes after that. My personal computer can't run this step at all without crashing (though every `spatialGE` step before this it can handle).
...
Questions on using this function:

* How do we know if a Moran's I value that is greater than 0 is significantly far from or greater than zero? Does this function compute p-values or help me to compute a z-score? Like should I use the mean genes value as the expected value and compute a z-score that way? If so, I think that's the best possible "analysis" we can have people do if we give them the dataframe ... have them compute the z-score.
